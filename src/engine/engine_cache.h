...
typedef struct eng_cache_node_s {
  // --- Hash Map Fields ---
  char *key;         // Key: "container:db:tag". Must be heap-allocated.
  ...
  int reference_count; // Tracks current users of the node.
  bool is_dirty;
} eng_cache_node_t;
...
typedef struct eng_cache_mgr_s {
  ...
  eng_cache_node_t *nodes_hash; // Head pointer for the uthash hash map.
  ...
  // --- Concurrency Control ---
  uv_rwlock_t lock; // A single lock to protect all cache operations.
} eng_cache_mgr_t;
