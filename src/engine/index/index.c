...
static db_put_result_t _index_put(MDB_dbi db, MDB_txn *txn,
  ...
  size_t used_size = mpack_writer_buffer_used(&writer);
  ...
  db_put_result_t res = db_put(db, txn, &db_key, data, used_size, false, true);
...
static bool _write_from_db(MDB_dbi dbi, index_write_reg_opts_t *opts,
  ...
  while ((r = db_cursor_get(src_cursor, &cursor_entry, op, NULL)) ==
         DB_CURSOR_OK) {
    op = MDB_NEXT;
...
bool index_open_registry(MDB_env *env, MDB_dbi dbi,
  ...
  // Initialize defs to prevent freeing random pointers on error
  memset(defs, 0, sizeof(defs));

  MDB_txn *read_txn = db_create_txn(env, true);
  if (!read_txn) {
    return false;
  }

  MDB_cursor *cursor = db_cursor_open(read_txn, dbi);
  if (!cursor) {
  ...
  while ((r = db_cursor_get(cursor, &cursor_entry, op, NULL)) == DB_CURSOR_OK &&
         def_count < MAX_NUM_INDEXES) {
  ...
  *key_to_index = kh_init(key_index);
  ...
  for (int i = 0; i < def_count; i++) {
    index_t index = {0};
    index.index_def = defs[i]; // Moves ownership of the allocated key
    ...
    if (!db_open(env, db_name, true, DB_DUP_KEYS_FIXED_SIZE_VALS,
                 &index.index_db)) {
      // If we can't open an index DB, the registry is inconsistent
      goto cleanup_on_failure;
    }

    int ret;
    khiter_t k = kh_put(key_index, *key_to_index, index.index_def.key, &ret);
    if (ret == -1) { // Hash put failed
      db_close(env, index.index_db);
      goto cleanup_on_failure;
    }

    kh_value(*key_to_index, k) = index;

    // Mark this index_def as "moved" so cleanup doesn't free it
    defs[i].key = NULL;
  }
...
cleanup_on_failure:
  // Clean up any keys that weren't successfully moved into the registry
  ...
  // If the hash map was partially populated, destroy it properly
  if (key_to_index && *key_to_index) {
    index_close_registry(env, key_to_index);
  }
