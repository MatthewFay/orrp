...
void consumer_cache_free_entry(consumer_cache_entry_t *entry) {
  ...
  // Don't free cc_bitmap, EBR will handle this

  free(entry);
}

static consumer_cache_entry_t *_create_entry(eng_container_db_key_t *db_key,
...
consumer_cache_create_entry_bitmap(eng_container_db_key_t *db_key,
  ...
  if (!cc_bitmap)
    return NULL;
  consumer_cache_entry_t *e = _create_entry(db_key, ser_db_key);
  if (!e)
    return NULL;
  atomic_init(&e->cc_bitmap, cc_bitmap);
  return e;
}
